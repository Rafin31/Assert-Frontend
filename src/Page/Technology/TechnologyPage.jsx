...
const TechnologyPage = () => {
  ...
  const { data: predictions = [], isLoading: loadingPredictions } = useQuery({
    ...
    queryFn: async () => {
      const res = await ServerApi.get('/userPrediction/predictions', { params: { realm: 'technology', status: 'approved' } });
  ...
  const { data: polls = [], isLoading: loadingPolls } = useQuery({
    queryKey: ['techPolls'],
    queryFn: async () => {
      const res = await ServerApi.get('/userPoll/poll', { params: { realm: 'technology', status: 'approved' } });
      return res.data.data || [];
    }
  });
  ...
  const sortedPredictions = sortItems(predictions.filter(p => p.question.toLowerCase().includes(searchTerm.toLowerCase())));
  const sortedPolls = sortItems(polls.filter(p => p.question.toLowerCase().includes(searchTerm.toLowerCase())));
  ...
  return (
    <div className="max-w-[1450px] mx-auto px-5 py-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-7">
        <div>
          <h1 className="text-3xl font-black text-slate-900">Technology</h1>
          <p className="text-slate-500 text-sm mt-1">Predictions and polls on technology topics.</p>
        </div>
